...
namespace AScript
{
	public class ScriptUtils
	{
		...
		public static readonly MethodInfo Method_ScriptContext_SetTempReadonly = typeof(ScriptContext).GetMethod("SetTempReadonly", new Type[] { typeof(string), typeof(object), typeof(Type), typeof(bool) });
		...
		/// </summary>
		/// <returns>对应的 Delegate 类型</returns>
		public static Type GetDelegateType(IList<Type> argTypes, Type returnType)
		{
			...
			try
			{
				if (returnType == typeof(void))
				{
					if (argTypes.Count == 0) return typeof(Action);
					if (argTypes.Count == 1) return typeof(Action<>).MakeGenericType(argTypes[0]);
					if (argTypes.Count == 2) return typeof(Action<,>).MakeGenericType(argTypes[0], argTypes[1]);
					if (argTypes.Count == 3) return typeof(Action<,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2]);
					if (argTypes.Count == 4) return typeof(Action<,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3]);
					if (argTypes.Count == 5) return typeof(Action<,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], argTypes[4]);
					if (argTypes.Count == 6) return typeof(Action<,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], argTypes[4], argTypes[5]);
					if (argTypes.Count == 7) return typeof(Action<,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], argTypes[4], argTypes[5], argTypes[6]);
					return null;
				}

				if (argTypes.Count == 0) return typeof(Func<>).MakeGenericType(returnType);
				if (argTypes.Count == 1) return typeof(Func<,>).MakeGenericType(argTypes[0], returnType);
				if (argTypes.Count == 2) return typeof(Func<,,>).MakeGenericType(argTypes[0], argTypes[1], returnType);
				if (argTypes.Count == 3) return typeof(Func<,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], returnType);
				if (argTypes.Count == 4) return typeof(Func<,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], returnType);
				if (argTypes.Count == 5) return typeof(Func<,,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], argTypes[4], returnType);
				if (argTypes.Count == 6) return typeof(Func<,,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], argTypes[4], argTypes[5], returnType);
				if (argTypes.Count == 7) return typeof(Func<,,,,,>).MakeGenericType(argTypes[0], argTypes[1], argTypes[2], argTypes[3], argTypes[4], argTypes[5], argTypes[6], returnType);
				return null;
			}
			catch
			{
