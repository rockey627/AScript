...
{
	public class MultiplyAssignOperator : IFunctionEvaluator, IFunctionBuilder
	{
		public static readonly MultiplyAssignOperator Instance = new MultiplyAssignOperator();
		...
		public void Build(FunctionBuildArgs e)
		{
				...
				|| !ExpressionUtils.ConvertMaxType(ref left, ref right))
			{
				...
				var addExpr = Expression.Dynamic(ExpressionUtils.Binder_Multiply, typeof(object), left, right);
				e.Result = Expression.Assign(left, addExpr);
			}
			else
			{
				e.Result = Expression.MultiplyAssign(left, right);
			}
		}
		...
		public void Eval(FunctionEvalArgs e)
		{
			...
			else if (arg0Node is OperatorNode opNode)
			{
				if (opNode.Name == "." && opNode.Right is VariableNode opRightNode)
				{
					...
					var value = ScriptUtils.GetAndSetValue(opLeftValue, opRightNode.Name, out var type0, (m, t, v) => (dynamic)v * (dynamic)arg1, e.Context.IsIgnoreCase() ?? false);
