...
namespace AScript.Lang.Python3.TokenHandlers
{
	...
	public class IfTokenHandler : ITokenHandler
	{
		...
		public void Build(DefaultSyntaxAnalyzer analyzer, TokenAnalyzingArgs e)
		{
			...
			ITreeNode bodyNode = Python3Lang.BuildSubBlock(e.CurrentToken.Column, analyzer, e.BuildContext, e.ScriptContext, createFullOptions, e.TokenReader, e.Control, e.Ignore, endTokens: Python3Lang.EndTokens);
			//
			...
			if (token.HasValue && token.Value.Column == e.CurrentToken.Column)
			{
				if (token.Value.Type != ETokenType.String && token.Value.Value == "elif")
				{
					...
					elseNode = analyzer.BuildOneStatement(e.BuildContext, e.ScriptContext, createFullOptions, e.TokenReader, e.Control, e.Ignore, endTokens: Python3Lang.EndTokens);
				}
				else if (token.Value.Type != ETokenType.String && token.Value.Value == "else")
				{
					analyzer.ValidateNextToken(e.TokenReader, ":");
					elseNode = Python3Lang.BuildSubBlock(e.CurrentToken.Column, analyzer, e.BuildContext, e.ScriptContext, createFullOptions, e.TokenReader, e.Control, e.Ignore, endTokens: Python3Lang.EndTokens);
				}
				else
				{
